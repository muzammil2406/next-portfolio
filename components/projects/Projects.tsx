...
interface Props {
    projects?: project[];
    projectsData: project[]; // Added projectsData property
}
...
const Projects = ({ projectsData }: Props) => {
    ...
    return (
        <SectionWrapper id="projects" className="mx-4 md:mx-0 min-h-screen">
            ...
            {/* Add ProjectsShowcase at the top */}
            <div className="my-10">
                <h3 className="text-2xl text-center mb-6">Featured Projects</h3>
                {/* <ProjectsShowcase projects={projects.slice(0, 3).map(p => ({
                    id: p.id,
                    title: p.title,
                    ...
                    category: p.category,
                    link: p.link,
                    image: p.image || "", // Provide a default or actual value
                    featured: p.featured || false, // Provide a default or actual value
                    projects: p.projects || [] // Provide a default or actual value
                }))} /> */}
            </div>
            ...
            <div className="overflow-x-auto scroll-hide md:w-full max-w-screen-sm mx-auto mt-6 flex justify-between items-center gap-2 md:gap-3 bg-white dark:bg-grey-800 p-2 rounded-md">
                {categories.map((c: string = "", i: number) => (
                    <span key={i} onClick={() => filterProjects(c)} className={`p-1.5 md:p-2 w-full text-sm md:text-base text-center capitalize rounded-md whitespace-nowrap ${category.toLowerCase() === c.toLowerCase() ? "bg-violet-600 text-white" : "hover:bg-gray-100 hover:dark:bg-grey-900"} cursor-pointer transition-all`}>
                        {c}
                    </span>
                ))}
            </div>
